...
- **Language registry** (`Support\Languages`): 18 target languages out of the box (Urdu, Arabic, Persian, Hindi, Marathi, Nepali, Bengali, Punjabi, Gujarati, Tamil, Telugu, Kannada, Malayalam, Sinhala, Russian, Greek, Amharic, Hebrew), each with native name, RTL flag, and Unicode script ranges compiled into both PHP and JS detection patterns. Fully overridable via the new `languages` config key.
